...
void driveLoop() {
    ...
    switch(currDriveState){
        ...
        // follow left wall
            ...
            else if (followRight){ // case where following right wall
                PIDinput = ldrRVal;
                polarity = -1;
    ...
    if (loop_count >= 100 && transmitDataToCloud){
        Serial.print(speedL);
        Serial.print(", ");
        Serial.print(speedR);
        Serial.print(", ");
...
void driftCorrection(){
    ...
    for (int i = 0; i < ARRAY_SIZE; i++){
        ...
        std::sprintf(left_wall_str, "%d", leftWallArr[dataIndex]);
        char right_wall_str[8];
        std::sprintf(right_wall_str, "%d", rightWallArr[dataIndex]);
