...
client_id = 'PicoW_' + beaconColour
user_t = 'BalanceBug'
password_t = '123'
topic_sub = beaconColour + '_beacon'
keep_alive=30
topic_pub = 'echo'
...
try:
    ...
    client.publish(topic_pub, beaconColour + ' beacon connected')
...
while True:
    try:
        ...
        if (time.time() - last_message) > keep_alive:
              client.publish('keepalive', "Keep alive message " + beaconColour)
              last_message = time.time()
