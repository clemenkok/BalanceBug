...
io.on("connection", (socket) => {
  ...
  socket.on("needball", (msg) => {
    ...
    if(currentcount1 < dataArr2.length){
      let tmp = [ dataArr2[currentcount1][0], dataArr2[currentcount1][1], "right"] 
      //console.log(tmp)
  ...
  socket.on("needbigball", (msg) => {
    if(currentcount2 < blurredarray.length){
      let tmp = [ blurredarray[currentcount2][0], blurredarray[currentcount2][1], "right"]
      io.emit("sendyouball", tmp);
      currentcount2++;
    }
  ...
  socket.on("getfakedata",(msg) =>{
    ...
    if(fakecurrentcount < fakedataArr.length){
      let tmp = [ fakedataArr[fakecurrentcount][0], fakedataArr[currfakecurrentcountentcount2][1], "left"]
      io.emit("sendyouball", tmp); 
      fakecurrentcount++;
    }
  })
